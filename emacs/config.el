...
(defun read-env-variable (variable-name)
  ...
  (with-temp-buffer
    ;; CHANGED: Point to home directory independent of the OS username
    (insert-file-contents (expand-file-name ".env" user-emacs-directory))
    (goto-char (point-min))
...
(defun buffer-binary-p (&optional buffer)
  "True if BUFFER or the current buffer is binary.
...
(defun nhexl-if-binary ()
  "If `nhexl-mode' is not already active, and the current buffer is binary, activate `nhexl-mode'."
  (interactive)
  (unless (eq major-mode 'nhexl-mode)
    (when (buffer-binary-p)
      (nhexl-mode))))

(add-hook 'find-file-hooks 'nhexl-if-binary)
...
(setq gnus-select-method '(nnimap "imap.gmail.com"
      ...
      gnus-secondary-select-methods '((nnimap "imappro.zoho.com"
					      (nnimap-stream starttls)
					      (nnmail-expiry-wait 90)))
